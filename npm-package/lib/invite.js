  ...
  return fetch(`https://api.github.com/orgs/${org}/invitations`, {
    ...
    body: `{"invitee_id":${user.id}}`
      ...
      if (response.status == 201) {
        spinner.stop();
        console.log(`Successfully invited GitHub user ${chalk.greenBright(user.login)} to ${chalk.greenBright(org)} GitHub organization`);
      } else {
        return response.json().then((data) => {
          spinner.stop();
          console.log(chalk.yellowBright(`Could not invite ${chalk.yellow(user.login)} to ${chalk.yellow(org)} because`));
